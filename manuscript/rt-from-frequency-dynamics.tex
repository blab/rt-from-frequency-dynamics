...
%TODO: Mention that in particular, we are concerned with the arrival of new genetic variants
...
In order to deal with the limitations of phylodynamic inference in this large sample size regime, previous studies have tried to estimate the growth of lineages using observed frequencies in sequenced SARS-CoV-2 samples. %TODO: Cite papers here?
...
In addition to the consistency in the inferred growth advantages between variants, we also observe similarities in the shapes of $R_{t}$ trajectories between states. %TODO: Expand this 
...
Summarize main results quickly. 
...
The underlying transmission model for this work is deterministic and does not account for demographic stochasticity and over-dispersion in transmission which is well documented in transmision transmission. 
% Introductions
Further, we do not explictly model multiple introductions of variants which can play an important role in variants establishing themselves in different geographies at low infection counts and could bias our estimates of the effective reproduction number if not properly accounted for.

%TODO: Hierarchical infernece
...
This would be a way to account for low growth advantages inferred for Beta (B.) in a handful of states including Louisiana and Wyoming which we believe is due to a lack successful introductions in these states.
...
Though there are several ways to improve these methods and explain their applicability, our current model does have use as a way of assessing early claims of variant advantages and is able to show there is evidence of consistent variant advantages shared between different geographies.
Additional work is needed to attribute these inferred advantages to biological mechanisms like immune escape and transmissibility using [HERE] or to changes in other factors such as contact patterns or non-pharameutical interventions. %TODO: With what data? \cite{}
...
With this in mind, our method potentially has use for evoluntary forecasting of variants for SARS-CoV-2. 
Extending the model towards this aim will require methods for quantifying population immunity as well escape potential for circulating and potential as SARS-CoV-2 tends towards endemicity.
...
%TODO: Re-run model with mean and ad aas below
