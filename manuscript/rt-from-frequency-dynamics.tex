...
\begin{abstract}
  Reconciling genetic data of SARS-CoV-2 lineages with epidemic surveillance data is ddiffcult.
...
In response to this, recent studies have tried to use the sample porportions of different variants as an approximation to the variant frequencies and to describe variant dynamics, but frequencies alone cannot capture the rich epidemiological behavior of SARS-CoV-2. 
...
Our method can be used to infer structured relationships between effective reproduction numbers across time series which may be amendable to various analyses including other features related to the effective reproductive number such as NPIs.
We use this model to quantify growth advantages for various SARS-CoV-2 variants of concern in the United States and estimate the effect of vaccination on curbing the spread of different variants in the United States.
...
As SARS-CoV-2 evolves, extant lineages may increase in their ability to transmit and escape acquired immunity. %TODO: Reference here 
...
Though it is typical to use phylodynamic methods to analyze genetic sequence data from epidemics, the sheer amount of data ([NUM OF SEQUENCES AS OF DATE]) makes these methods hard to use at the scale of interest due to computational constraints associated with large numbers of sequences.
In order to deal with the limitations of phylodynamic inference in this large sample size regime, previous studies have estimated the growth of lineages using observed frequencies in sequenced SARS-CoV-2 samples. %TODO: Cite papers here?
...
To better capture epidemiological dynamics, there are methods which describe the growth of total infections using confirmed case, hospitalization, or death data to estimate the effective reproduction number, the average number of infections a single infectious individual generates, in a given outbreak.
Though, these methods are excellent for describing overall growth rates, they cannot capture the evolutionary dynamics and fitness changes between different variants since they often assume the population dynamics are described by a singular $R_{0}$ value or $R_{t}$ trajectory. \cites{Cori2013, Abbott2020} % Might be a couple other papers
...
We develop joint epidemiological and population genetic model of SARS-CoV-2 to assess the growth of different lineages over time and infer differences in the effective reproduction numbers of SARS-CoV-2 variants as well as underlying frequency of variants under noisy sampling.
...
We estimate the effective reproduction numbers of SARS-CoV-2 variants of concern in the United States using daily confirmed case counts and sequence lineage annotations from GISAID. %TODO: Be more clear about PANGO and where these came from
...
Modeling the effect of changes in other factors such as contact patterns or non-pharmaceutical interventions can be done with the current formualation of the model by including quantities of interest as features in the $R_{t}$ model. 
