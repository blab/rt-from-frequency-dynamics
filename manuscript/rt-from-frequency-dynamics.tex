...
\begin{abstract}
Reconciling genetic data of SARS-CoV-2 variants with epidemic surveillance data is difficult.
Models of infectious diseases using genomic data typically rely on phylogenetic and phylodynamic inference which have issues scaling to large number of samples and which are often not designed to estimate fitness differences between variants. 
In response to this, recent studies have used the sample proportions of different variants in sequencing data as an approximation to the variant frequencies in order to describe variant frequency dynamics, but frequencies alone cannot capture the rich epidemiological behavior of SARS-CoV-2.
...
%TODO: What is really the point of meaning phylodynamics? To swat away comments on why we don't use them for this particular problem.
...
With this fixed growth advantage model, we parameterize fitness of variants at the level of transmission by inferring variant-specific effective reproduction numbers. This differs from previous work on variant effective reproduction numbers which mostly deal with only two variants or parameterize these differences by assuming logistic growth of frequencies \cite{Vhringer2021}. %TODO: I think there's more references here.
...
In addition to the consistency in the inferred growth advantages between variants, we also observe similarities in the shapes of $R_{t}$ trajectories between states. This can be seen in figure \ref{fig:4} %TODO: Expand this
...
%TODO: Reinforce extensibility of the model with the structured R_{t} stuff
