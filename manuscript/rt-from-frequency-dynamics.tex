...
We implement two models of variant-specific effective reproduction number based on a renewal equation framework of epidemic spread (see Methods), a fixed growth advantage model and a time-varying growth advantage model (GARW).
...
%TODO: Update model descriptions
...
The GARW model is shown in Figure \ref{fig:GARW_rt_Washington} and the fixed growth advantage model is shown in Figure \ref{fig:fixed_growth_Washington}. Example model output from several other states is provided in the supplemental appendix.
...
We use a cutoff of 2000 sequences from a particular variant across states to determine threshold of circulation. %TODO: Update clades and sequence cut off.
...
We use a cutoff of 5000 sequences from a particular state as basis for including the state in the dataset. %TODO: Update cut off and number of states
...
%TODO: Add discussion on Omicron and citations.

%TODO: Put in new figures. Here goes time varying growth advantages
\begin{figure}[t]
  \centering
...
%TODO: Need text associated with wave size analysis
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/wave_size_to_wave_size.png}
  \caption{\textbf{Comparing sizes of subsequent waves.}
    (a) Plotting estimated Alpha wave size agaist Delta wave size in individual states. Error bars denote 80\% credible intervals.
    (b) Plotting estimated Delta wave size agaist Omicron wave size in individual states. Error bars denote 80\% credible intervals.
  }
...
\paragraph{Estimating time varing growth advantages}
...
Here, we subsetted to sequences with specimens collected from the USA between January 1, 2021 and October 1, 2021. %TODO:  Update this section.
...
However, we reduced dataset to just the 34 states with more 5000 sequences available in this timeframe.
...
Here subsripts denote the generation time used when computing $R_{t}$.
