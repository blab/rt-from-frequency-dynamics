...
Note that this analysis does not say much about vaccination itself and in fact vaccine-induced protection against infection has been shown for Delta [REF]. %TODO: What reference should go here?
Further, we see that repeating this analysis using state population with a booster vaccine and wave size for Omicron shows no clear trend.
These results highlight the complicated relationship between variant sucess and population immunity as it is clear that single immunity sources or one-off transmission advantages can not completely explain the observed variation in Omicron wave.

Standard compartment models of epidemic suggest that a higher initial variant $R_{t}$ will lead to larger wave sizes assuming the mechanism for increase is due to intrinsic transmissibility. 
...
\begin{figure}[t]
  ...
  (a) Posterior wave size as a function of the proportion vaccined at time of first sucessful variant introduction. Colors of points above correspond to each variant as shown in \ref{fig:GARW_rt_Washington}.
...
\begin{figure}[t]
  ...
  Posterior estimates of variant-specific wave size against $R_{t}$ at time of first sucessful introduction.
...
We estimate variant-specific wave sizes using the posterior estimates of incidnce. We define that the wave size attributable to variant $v$ is
 \begin{align*}
...
For the purposes of the wave size analyses in Figures \ref{fig:vaccination_to_wave_size} and \ref{fig:intro_R_to_wave_size}, we define the first sucessful introduction to be the first time point at which variant $v$ is above a threshold frequency and have effective reproduction number greater than one.
...
Therefore, by furhering assuming that the variant infections are exponentially growing with rates $r_{v}$, we can then identify $\beta_{v} = r_{v} - r_{u_{*}}$.
