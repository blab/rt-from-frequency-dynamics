...
In both models, variants generate infections independently of one another, but the sum of infections across all variant categories is observed through surveillance data like case counts or hospitalizations. In order to disaggregate this infections by variant we rely on frequency estimates which are informed by counts of sequenced genetic variations using a Dirichlet-Multinomial likelihood.
...
With this fixed growth advantage model, we parameterize fitness of variants at the level of transmission by inferring variant-specific effective reproduction numbers. This differs from previous work on variant effective reproduction numbers which often parameterize these differences by assuming logistic growth of frequencies \cite{Earnest2021, Vhringer2021}. %TODO: I think there's more references here.
...
We test these models on data from the United States showing example model output from Washington state. 
The free $R_{t}$ model can be seen in figure \ref{fig:1} and the fixed growth advantage model can be visualized in figure \ref{fig:2}. 
...
We estimate the effective reproduction numbers of SARS-CoV-2 variants of concern in the United States using daily confirmed case counts obtained from the US CDCs and sequences shared via the Nextstrain-curated `open' dataset \cite{Hadfield2018} (see Data and code accessibility). 
...
In order to inform our estimates of the frequency of genetic variants, we divide sequences from each state into daily sample counts for each of the 7 variants above and one "other" category containing all other non-variant sequences.
...
We find that overall there appears to be consistent trends in the effective reproduction numbers of variants across the United States (see figure \ref{fig:4}). 
...
Further, these growth advantages appear to be consistent between the states analyzed (see figure \ref{fig:3}).

Overall, in all states, we find that the Delta variant appears to have the highest growth advantage of all variants considered with median growth advantge of [FILL] followed by Mu which had a median growth advantage of [FILL].

The inferred transmission advantage of Delta appears to be relatively consistent with the approaches of % Fritz, Moritz, others.
% which estimate what advantages ...
...
In order to quantify this source of error, we derive a equation relating our inferred growth advantages, the epidemic growth rates, and the mean and standard deviation of the generation time distribution.
There is also a needed to account for biases in the case data which may not faithfully describe the infection dynamics of SARS-CoV-2 due to changes in case ascertainment rate, as possibly caused by differences in testing intensity, infection severity among other reasons.
...
Using hierarchical models of variants to jointly estimate growth advantages and pool estimates across locations could be a useful approach for analyzing consistency between growth advantages of lineages geographically and beginning to combat the issue of multiple introduction events.
Though, to fully combat this issue, would likely involve incorporating demographic stochastic into the model at the level of transmission and reduce its speed of inference, scalability, and limit available inference options.
...
Modeling the effect of changes in other factors such as contact patterns or non-pharmaceutical interventions can be done with the current formulation of the model by including quantities of interest as features in the $R_{t}$ model as in \cite{Sharma2021}.
...
\begin{figure}
  ...
  \caption{Using fixed growth advantage model, we infer growth advantages for 7 variants in 39 US states.
...
\begin{figure}
  ...
  \caption{Inferred effective reproduction numbers from free growth model in 39 states show a consistent trend.}%
  \label{fig:4}
\end{figure}
