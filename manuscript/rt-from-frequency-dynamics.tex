...
\begin{figure}[h!]
  ...
  \tbc{A (larger, colored) dot at the median of each violin plot would make this panel easier to read. Some of the lines are quite thin.}
  }
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Fitting the GARW model to Washington state data.}
    ...
    \tbc{Thanks for humoring me with the figure adjustments. The only thing I'd recommend here is giving each panel a separate letter a--f. As it stands it was hard for me to follow which parts of legend correspond to which panels in the figure.}
    }
...
Example model output from several other states is provided in the supplemental appendix in Figures \ref{fig:GARW_rt_California}, \ref{fig:GARW_rt_Florida}, \ref{fig:GARW_rt_Michigan}, and \ref{fig:GARW_rt_New-York}
...
%TODO: Clean and correct this for updated posteiors.
...
We can also see that states with larger Delta waves tended to have larger Omicron waves as well (Fig.\ \ref{fig:wave_size_to_wave_size}b).
\tbc{I'm pretty concerned that the apparent positive correlation between larger Delta waves and larger Omicron waves is an artifact of state-level differences in reporting rate. I can't think of an easy solution for this issue. If we decide to keep, this result should be heavily caveated.}
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Comparing sizes of subsequent waves.}
    ...
    \tbc{Each of these panels should have $r$ or $R^2$ and $p$ listed in the top-left corner.}
    \tbc{This figure should be more interpretable if the axes were in natural units of 0, 2.5\%, 5\%, etc...}
  }
  \label{fig:wave_size_to_wave_size}
\end{figure}

Theory suggests that having a higher proportion of a population with prior immune exposure would ultimately lead to smaller wave sizes [REF].
...
For some variants such as Gamma, Iota, and Mu, we see a decline in relative wave size as a function of vaccination.
Largely, for Delta and Omicron, this effect is less clear.
Note that this analysis does not say much about vaccination itself and in fact although Delta showed partially reduced vaccine effectiveness against infection, studies still found significant protection [REF]. %TODO: What reference should go here?
Further, we see that repeating this analysis using state population with a booster vaccine and wave size for Omicron shows no clear trend (Fig.\ \ref{fig:vaccination_to_wave_size}b).
...
\tbc{Current Figure 7a seems like a perfect illustration of Simpson's paradox. The surface reading is that more vaccination results in larger waves, but this is due to the fitter Delta and Omicron variants emerging after widespread vaccination. I think it would be more honest and interpretable to panel out individual variants. Take each variant and do the same plot of proportion vaccinated vs wave size. Include $r$ or $R^2$ for each panel. Include Omicron booster current Figure 7b as just another panel in this series. Feel free to drop uninformative variants (I think basically everything's zero for Alpha, Beta and Epsilon). I think this leaves 6 panels (Gamma, Iota, Mu, Delta, Omicron, Omicron boosters).}
...
\begin{figure}[h!]
  ...
  (a) Posterior wave size as a function of the proportion vaccinated at time of first successful variant introduction. Colors of points above correspond to each variant as shown in \ref{fig:GARW_rt_Washington}.
  (b) Posterior wave size for Omicron as a function of the proportion of the population with a booster vaccine.
  \tbc{This figure should be more interpretable if the axes were reported as fractions 0, 20\%, 40\%, etc... rather than proportions.}
}
...
\tbc{Most of the signal here is coming from Omicron vs previous variants. It seems a little wonky that Delta gets estimated as having a consistently higher growth rate in the fixed model (Figure 4), but it's initial $R_t$ is often quite low (Figure 3). A couple thoughts: (1) How about increasing the stringency for when to consider `initial' $R_t$ to be a greater sequence fraction? (2) What would the relationship between fixed growth advantage from Figure 4 and wave size look like? Here, I'm really hoping for a decently predictive metric that can predict variant impact. This has seemed to be a key question with BA.5, etc...}

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{figs/intro_R_to_wave_size.png}
  \caption{\textbf{Visualizing $R_{t}$ at introduction against posterior variant-specific wave size}
  Posterior estimates of variant-specific wave size against $R_{t}$ at time of first successful introduction.
  \tbc{I think the two outliers for Alpha and Epsilon are due to model taking a couple weeks to calibrate. The overly large initial $R_t$ can be seen in Figure 3 where it takes just a couple of weeks to settle. We may want to only start estimates from Jan 15 even if this dataset starts at Jan 1, 2021.}
  \tbc{The x axis should be `Variant $R_t$ at introduction' and I think the y axis should just be `Variant wave size'.}
  }
...
\tbc{It would be good to somehow mention results from Figures 6--8 here as well.}
...
As an example, we assess the rise of Omicron sublineage BA.2 in the United States in Figure \ref{fig:BA_2_growth_advantages}.
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Estimating variant growth advantages relative to BA.1 in 16 US states.}
    (a) Time-varying growth advantages for Delta and BA.2 relative to BA.1 using GARW model. Histograms denote the distribution of the variant growth advantages across all times.
    ...
    \tbc{I think you can just drop Delta from this figure. The key question is BA.2 relative to BA.1. Just like we generally ignore `Other' I think appropriate to ignore Delta in this presentation.}
  }
...
Extending the model further towards this aim will likely require methods for quantifying various sources population immunity as well as escape potential for circulating and emerging SARS-CoV-2 variants as a way to explain these growth advantages using data.
...
For the purposes of the wave size analyses in Figures \ref{fig:vaccination_to_wave_size} and \ref{fig:intro_R_to_wave_size}, we define the first successful introduction to be the first time point at which variant $v$ is above a threshold frequency and have effective reproduction number greater than one.
\tbc{What's this threshold frequency?}
