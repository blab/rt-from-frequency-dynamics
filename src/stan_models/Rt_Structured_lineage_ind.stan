...
transformed data {
    ...
    /* vector[N_lineage] I0_max = rep_vector(3./7. * sum(cases[1:7]), N_lineage); */
    ...
    // Compute weekend matrix
    matrix[L_wf, 7] wk_mat = get_weekend_mat(L_wf);
}

parameters {
  matrix[K, N_lineage] b;  // beta coeffiecients for features
  ...
  /* real<lower=0, upper=1> xi; // Overdispersion relative to multinomial */
}
...
transformed parameters {
  ...
  matrix[L_wf, N_lineage] R = exp(features * b);
  vector[L_wf] rho_vec = wk_mat * to_vector(rho);
  real<lower=0> inv_alpha = inv_square(zalpha);
  /* real trans_xi = inv(xi) - 1; */
