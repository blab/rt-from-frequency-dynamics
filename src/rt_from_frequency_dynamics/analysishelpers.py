...
from rt_from_frequency_dynamics import get_R, get_growth_advantage, get_growth_advantage_time, get_little_r, get_little_r_gen, get_I, get_freq
...
def fit_SVI(VD, LM, opt, iters=100_000, num_samples = 1000, path = ".", name="Test", load=False, save=False):
...
def gather_R(MP, ps, forecast=False):
    R_dfs = []
    for name, p in MP.locator.items():
        R_dfs.append(pd.DataFrame(get_R(p.dataset, p.data, ps, name, forecast=forecast)))
    return pd.concat(R_dfs)

def gather_little_r_gen(MP, ps, g, forecast=False):
    r_dfs = []
    for name, p in MP.locator.items():
        r_dfs.append(pd.DataFrame(get_little_r_gen(p.dataset, g, p.data, ps, name, forecast=forecast)))
    return pd.concat(r_dfs)

def gather_little_r(MP, ps, forecast=False):
    r_dfs = []
    for name, p in MP.locator.items():
        r_dfs.append(pd.DataFrame(get_little_r(p.dataset, p.data, ps, name, forecast=forecast)))
    return pd.concat(r_dfs)

def gather_ga(MP, ps, rel_to="other"):
    ga_dfs = []
    for name, p in MP.locator.items():
        ga_dfs.append(pd.DataFrame(get_growth_advantage(p.dataset, p.data, ps, name, rel_to=rel_to)))
    return pd.concat(ga_dfs)

def gather_ga_time(MP, ps, rel_to="other"):
    ga_dfs = []
    for name, p in MP.locator.items():
        ga_dfs.append(pd.DataFrame(get_growth_advantage_time(p.dataset, p.data, ps, name, rel_to=rel_to)))
    return pd.concat(ga_dfs)

def gather_I(MP, ps, forecast=False):
    I_dfs = []
    for name, p in MP.locator.items():
        I_dfs.append(pd.DataFrame(get_I(p.dataset, p.data, ps, name, forecast=forecast)))
    return pd.concat(I_dfs)

def gather_freq(MP, ps, forecast=False):
    freq_dfs = []
    for name, p in MP.locator.items():
        freq_dfs.append(pd.DataFrame(get_freq(p.dataset, p.data, ps, name, forecast=forecast)))
