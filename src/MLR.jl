...
function MLRData(seq_df::DataFrame)
    ...
    return MLRData(seq_counts, seq_names, dates)
end
...
function MLRModel(max_T::T) where {T <: Real}
    features = hcat(ones(T, max_T), collect(1:max_T))
    return MLRModel{T}(features)
end
...
function make_stan_model(LM::MLRModel, model_name::String, tmpdir)
